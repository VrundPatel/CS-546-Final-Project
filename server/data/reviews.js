...
const createReview = async (
    ...
    let newOverallRating = ((restroomThatReviewed.overallRating * (updatedReviews.length - 1) + newReview.rating) / updatedReviews.length).toFixed(1);
    await restroomsCollection.updateOne({ _id: ObjectId(restroomId) }, { $set: { reviews: updatedReviews } });
    await restroomsCollection.updateOne({ _id: ObjectId(restroomId) }, { $set: { overallRating: newOverallRating } });

    const usersCollection = await users();
    await usersCollection.updateOne({ _id: ObjectId(userId) }, { $push: { reviewIds: String(newReview._id) } });
...
const removeReviewById = async (reviewId) => {
    ...
    if (newReviewsList.length === 0) {
        ...
        newOverallRating = ((currentRestroom.overallRating * (newReviewsList.length + 1) - currentRestroom.reviews[index].rating) / newReviewsList.length).toFixed(1);
    }

    await restroomsCollection.updateOne({ _id: ObjectId(currentRestroom._id) }, { $set: { reviews: newReviewsList } });
    await restroomsCollection.updateOne({ _id: ObjectId(currentRestroom._id) }, { $set: { overallRating: newOverallRating } });
    const curReview = await this.getReview(reviewId);
    const usersCollection = await users();
    await usersCollection.updateOne({ _id: ObjectId(curReview.userId) }, { $pull: { reviewIds: reviewId } });
